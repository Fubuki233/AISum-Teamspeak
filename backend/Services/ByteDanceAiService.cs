...
public class ByteDanceAiService
{
    ...
    7. 默认优先返回 NEW，新建一条新的总结。
    8. 只有在上一条总结是刚刚生成的，且最新对话只是对同一件事的补充、续写、纠正或情绪延伸时，才返回 UPDATE。
    9. 如果拿不准，一律返回 NEW；不要频繁改写旧总结。
    10. 总结内容应更像『局势摘要』或『这一小段时间发生了什么』，而不是聊天记录摘抄。
    11. 尽量压缩成 2 到 4 句，信息密度高，少废话。
    12. 返回格式固定：先输出 UPDATE 或 NEW，然后换行输出总结内容。";
...
请不要复述原话，而要概括：他们在推进什么事、进展如何、谁的情绪最明显、整体氛围怎样。";
