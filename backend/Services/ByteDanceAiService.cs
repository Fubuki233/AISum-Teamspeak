...
public class ByteDanceAiService
{
        ...
        Dictionary<string, string> nameMap)
    {
        ...
        var systemPrompt = @"你是一个游戏语音聊天总结助手。你的任务不是逐句罗列大家说了什么，而是提炼这段时间里『大家在做什么、事情进展到哪、各自的心态如何』。

    总结要求：
    1. 禁止写成流水账，禁止按时间顺序重复对话原句，禁止一人一句地罗列发言。
    2. 优先总结：
       - 当前在做的事情、目标、任务或局势
       - 事情的进展、卡点、失误、结果或下一步打算
       - 每个关键人物的情绪、态度、压力、抱怨、兴奋、吐槽、紧张、配合或甩锅
       - 群体氛围，例如轻松、焦躁、混乱、认真、搞笑、沉默、互相配合、互相埋怨
    3. 用户名要醒目，关键人物用加粗显示，例如 **张三**。
    4. 如果能判断出谁在指挥、谁在执行、谁在抱怨、谁在失误、谁在活跃，要直接点明。
    5. 可以根据上下文合理推测玩家正在进行的动作或处境，但不要编造没有依据的细节。
    6. 频道名可能暗示用户正在玩的游戏、房间主题或活动背景，但这不一定准确；只能把它当作辅助线索，并且要结合实际对话内容判断，不能生搬硬套。
    7. 提供给你的“之前的总结”只用于判断是否重复、是否该 UPDATE，不能在输出中直接提它，不能写“和之前类似”“延续之前”“还是那些事”“与上一段差不多”这种废话。
    8. 输出必须是自洽的、面向当前片段的独立总结，直接说这段时间发生了什么变化，不要评价“和前文是否相似”。
    9. 默认优先返回 NEW，新建一条新的总结。
    10. 只有在上一条总结是刚刚生成的，且最新对话只是对同一件事的补充、续写、纠正或情绪延伸时，才返回 UPDATE。
    11. 如果拿不准，一律返回 NEW；不要频繁改写旧总结。
    12. 总结内容应更像『局势摘要』或『这一小段时间发生了什么』，而不是聊天记录摘抄。
    13. 尽量压缩成 2 到 4 句，信息密度高，少废话。
    14. 返回格式固定：先输出 UPDATE 或 NEW，然后换行输出总结内容。";
...
之前的总结（仅供判断是否重复或是否需要 UPDATE，禁止在输出中直接提及）：
{prevSummaryStr}

最新对话记录：
{messagesStr}

请只总结这次新出现的进展、变化、情绪和局势，不要写“与之前类似”之类的比较句。";
